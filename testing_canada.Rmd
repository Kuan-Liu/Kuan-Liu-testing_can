...
- **Highlights**
  ...
  - **Quebec has rapidly increased its testing capacity. The total number of tests per 100,000 population has increased to 1109 as of Apr 4, 2020, comparing to 365 on Mar 25, 2020.**
...
map_test = tm_shape(testing_data)+tm_polygons(col="test",breaks=c(0,1000,10000,50000,100000, Inf),labels = c("<1000", "1000-10000", "10000-50000","50000-100000",">100000"), title=" ",palette ="YlOrRd")+tm_layout(title="Tests", title.size = 1,title.position=c("left","bottom"),legend.position = c(0.73,0.7))

#map stat can be grouped with fixed breaks or continuous, 
map_case = tm_shape(testing_data)+tm_polygons(col="case",breaks=c(0,1,100,500,1000,2000,Inf),labels = c("0", "1-100","100-500","500-1000","1000-2000",">2000"),title=" ",palette ="YlOrRd")+tm_layout(title="Cases", title.size = 1,title.position=c("left","bottom"),legend.position =c(0.77,0.67))

map_test_p = tm_shape(testing_data)+tm_polygons(col="test_pop",breaks=c(0,500,1000,1500,Inf),labels = c("0-500", "500-1000","1000-1500",">1500"), title=" ",palette ="YlOrRd")+tm_layout(title="Tests per 100,000", title.size = 1,title.position=c("left","bottom"),legend.position =c(0.78,0.76))

map_case_p = tm_shape(testing_data)+tm_polygons(col="case_pop",breaks=c(0,1,15,30,45,Inf),labels = c("0", "1-15", "15-30","30-45",">45"), title=" ",palette ="YlOrRd")+tm_layout(title="Cases per 100,000", title.size = 1,title.position=c("left","bottom"),legend.position = c(0.84,0.7))
...
geom_line(size = 1)+geom_point()+
  ...
  scale_y_continuous(limits = c(0, 3100), breaks = seq(0,3100,500))+
  scale_color_manual(values=teamcolour_territory)+
  scale_x_date(date_breaks="1 day",date_labels="%b %d")+
...
#Province plot;
testing_cum2<-subset(testing_cum, Province %in% c("AB","BC","MB","NB","NL","NS","ON","PE","QC","SK")& dateformat > as.Date("2020-03-19") & dateformat < as.Date("2020-04-07"))
...
cowplot::plot_grid(topp, botp, nrow=2, rel_heights = c(0.5,0.5),align = "v")

#Territory plot;
testing_cumt<-subset(testing_cum, Province %in% c("NU","YT","NT") & dateformat > as.Date("2020-03-19") & dateformat < as.Date("2020-04-07"))
...
- **Highlights**
  ...
  - **Quebec has the highest proportion of positive tests (8.7%).**
  - **There appears to be a back log of test reporting in Prince Edward Island and Nunvaut. The back log in Ontario is observed to be cleared. The proportion of pending tests in Ontario has reduced significantly, at 0.8% now comparing to the same proportion on Mar 25, 2020 at 29.4%.**
