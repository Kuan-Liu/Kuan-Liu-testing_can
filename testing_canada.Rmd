...
- **Highlights**
  ...
  - **Quebec has rapidly increased its testing capacity. The total number of tests per 100,000 population has increased to 623 as of Mar 28, 2020 comparing to 365 on Mar 25, 2020.**
...
cump<-ggplot(dfp, aes(x = forcats::fct_rev(Province), y = Total, fill = Data)) +
  ...
  coord_flip() + 
  labs(y = "", x = " ", title = "As of March 30, 2020") +
...
kable(testing_todate_p[,1:7], caption = "Reported testing data province and territory, public data accessed online as of March 30, 2020 (8 pm EDT).", row.names = FALSE, align=c("c","c","c","c","c","c","c"))
...
#### Geographic mapping on the total number of tests, total number of cases, total number of tests per 100,000 population and total number of cases per 100,000 population
...
map_test = tm_shape(testing_data)+tm_polygons(col="test",breaks=c(0,1000,5000,10000,40000, Inf),labels = c("<1000", "1000-5000", "5001-10000","10001-40000",">40000"), title=" ",palette ="YlOrRd")+tm_layout(title="Tests", title.size = 1,title.position=c("left","bottom"),legend.position = c("right","top"))
...
map_case = tm_shape(testing_data)+tm_polygons(col="case",breaks=c(0,1,50,100,500,1000,2000,Inf),labels = c("0", "1-50", "51-100","101-500","501-1000","1001-2000",">2000"),title=" ",palette ="YlOrRd")+tm_layout(title="Cases", title.size = 1,title.position=c("left","bottom"),legend.position = c("right","top"))

map_test_p = tm_shape(testing_data)+tm_polygons(col="test_pop",breaks=c(0,400,700,1000,Inf),labels = c("0-400", "401-700", "701-1000",">1000"), title=" ",palette ="YlOrRd")+tm_layout(title="Tests per 100,000", title.size = 1,title.position=c("left","bottom"),legend.position = c("right","top"))

map_case_p = tm_shape(testing_data)+tm_polygons(col="case_pop",breaks=c(0,1,10,20,Inf),labels = c("0", "1-10", "11-20",">20"), title=" ",palette ="YlOrRd")+tm_layout(title="Cases per 100,000", title.size = 1,title.position=c("left","bottom"),legend.position = c("right","top"))

tmap_arrange(map_test, map_case,map_test_p, map_case_p,nrow=2,ncol=2)

```

### 2.3.1 Estimated Test-Case Ratio
We define the test-case ratio (TCR) as
$$
TCR= \frac{\text{Total number of postive cases}}{\text{Total number of tests completed}},
$$
which is also known as the portion of positives cases. For provinces and territories that reported total number of tests including under investigation tests. The demoninator has be asjusted to inclunde only the completed, results known tests.
...
ggplot(testing_todate, aes(x=total_adjp, y=`Total cases per 100,000`, colour=Province)) +
  ...
  annotate(geom="text", x=1000, y=22, label="national test-case ratio at 4%",
              color="red") +
  ...
  labs(y="\n Total cases per 100,000 popualtion", x=" \n Total tests per 100,000 population",title = "Cases per 100,000 by tests per 100,000, as of March 30, 2020")+
  theme(panel.background = element_rect(fill = "white", colour = "white"),
        panel.grid.major = element_line(colour = "grey95"),
        panel.grid.minor = element_line(colour = "grey95"),
...
geom_line(size = 1)+geom_point()+
  scale_y_continuous(limits = c(0, 66000), breaks = seq(0,66000,5000))+
...
- **Highlights**
  ...
  - **Given the most recent data, Quebec completes on average 7,000 tests per day. British Columbia and Alberta complete on average 3,000 tests per day. Ontario completes around 5,000 to 6,000 tests per day.**
...
#Province plot;
testing_cum2<-subset(testing_cum, Province %in% c("AB","BC","MB","NB","NL","NS","ON","PE","QC","SK")& dateformat > as.Date("2020-03-19") & dateformat < as.Date("2020-03-30"))
...
#Territory plot;
testing_cumt<-subset(testing_cum, Province %in% c("NU","YT","NT") & dateformat > as.Date("2020-03-19") & dateformat < as.Date("2020-03-28"))
...
- **Highlights**
  ...
  - **Newfoundland and Labrador has the highest proportion of positive tests (6.3%). This can be interpreted as for every 100 tests completed (with known results) in Newfoundland and Labrador, we expect around 6 of them to be postive.**
  - **There appears to be a back log of test reporting in Ontario and Prince Edward Island. However the back log in Ontario is observed to be decreasing. The proportion of pending tests in Ontario on Mar 30, 2020 is 11.7% comparing to the same proportion on Mar 25, 2020 at 29.4%.**
...
ggplot(new.testing_cum, aes(x = Rank, group = Province, 
  ...
  gganimate::transition_states(dateformat, transition_length = 8, state_length = 4) +
