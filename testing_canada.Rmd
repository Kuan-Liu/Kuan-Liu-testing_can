...
map_test_p <- tm_shape(testing_data) +
  tm_polygons(col = "test_pop",
              breaks = c(1000, 2000, 3000, 4000, 5000,6000,Inf),
              labels = c("1000-2000", "2000-3000", "3000-4000","4000-5000","5000-6000",">6000"), 
              title = " ",
              palette = "YlOrRd") + 
...
ggplot(testing_todate, aes(x=total_adjp, y=`Total cases per 100,000`, colour=Province)) +
  ...
  annotate(geom="text", x=1300, y=650, label=paste("national test-case ratio at ", TCR_CAN*100 ,"%"),
              color="red") +
  scale_x_continuous(limits = c(0, 6500), breaks = seq(0,6500,500))+
...
testing_cum2<-subset(testing_cum, Province %in% c("AB","BC","MB","NB","NL","NS","ON","PE","QC","SK")& dateformat > as.Date("2020-03-31"))

p_testcum<-ggplot(testing_cum2, aes(x=dateformat, y=cumulative, colour=Province)) +
geom_line(size = 1) + 
  geom_point() +
  scale_y_continuous(limits = c(0, 840000), breaks = seq(0, 840000, 20000)) +
...
geom_line(size = 1)+geom_point()+
  ...
  scale_y_continuous(limits = c(0,6600), breaks = seq(0,6600,200))+
  scale_color_manual(values=teamcolour_province)+
  scale_x_date(date_breaks="1 day",date_labels="%b %d")+
...
testing_cum2<-subset(testing_cum, Province %in% c("NU","YT","NT") & dateformat > as.Date("2020-03-31"))
...
#Province plot;
testing_cum2<-subset(testing_cum, Province %in% c("AB","BC","MB","NB","NL","NS","ON","PE","QC","SK")& dateformat > as.Date("2020-03-31") & dateformat < Sys.Date())
...
topp<-ggplot(testing_cum_top, aes(x=dateformat, y=est_daily2, colour=Province)) +
  ...
  scale_y_continuous(limits = c(0, 22000), breaks = seq(0,22000,1000))+
...
#Territory plot;
testing_cumt<-subset(testing_cum, Province %in% c("NU","YT","NT") & dateformat > as.Date("2020-03-31") & dateformat < Sys.Date())
