...
- **Highlights**
  ...
  - **Nova Scotia has significantly increased its testing number after the Easter long weekend, now ranking as the number one province based on testing capacity per capita.**
  - **Quebec has rapidly increased its testing capacity. The total number of tests per 100,000 population has increased to 1582 as of Apr 13, 2020, comparing to 365 on Mar 25, 2020.**
  - **Northwest territories and Yukon reported over 2000 tests per 100,000 population, highest in Canada.**
...
map_test_p <- tm_shape(testing_data) +
  tm_polygons(col = "test_pop",
              breaks = c(500, 1000, 1500, 3000, Inf),
              labels = c("500-1000", "1000-1500", "1500-3000", ">3000"), 
              title = " ",
              palette = "YlOrRd") + 
...
ggplot(testing_todate, aes(x=total_adjp, y=`Total cases per 100,000`, colour=Province)) +
  ...
  annotate(geom="text", x=500, y=200, label=paste("national test-case ratio at ", TCR_CAN*100 ,"%"),
              color="red") +
  scale_x_continuous(limits = c(0, 2500), breaks = seq(0,2500,500))+
  scale_y_continuous(limits = c(0, 220), breaks = seq(0,220,10))+
...
geom_line(size = 1) + 
  geom_point() +
  scale_y_continuous(limits = c(0, 180000), breaks = seq(0, 180000, 20000)) +
...
- **Highlights**
  ...
  - **Quebec has the highest proportion of positive tests (11%).**
  - **There appears to be a back log of test reporting in Nunvaut. The back log in Ontario is observed to be cleared. The proportion of pending tests in Ontario has reduced significantly, at about 5% now comparing to the same proportion on Mar 25, 2020 at 29.4%.**
