...
ggplot(testing_todate, aes(x=total_adjp, y=`Total cases per 100,000`, colour=Province)) +
  ...
  annotate(geom="text", x=800, y=400, label=paste("national test-case ratio at ", TCR_CAN*100 ,"%"),
              color="red") +
  scale_x_continuous(limits = c(0, 4300), breaks = seq(0,4300,500))+
...
geom_line(size = 1) + 
  geom_point() +
  scale_y_continuous(limits = c(0, 350000), breaks = seq(0, 350000, 20000)) +
...
#Province plot;
testing_cum2<-subset(testing_cum, Province %in% c("AB","BC","MB","NB","NL","NS","ON","PE","QC","SK")& dateformat > as.Date("2020-03-21") & dateformat < Sys.Date())
...
topp<-ggplot(testing_cum_top, aes(x=dateformat, y=est_daily2, colour=Province)) +
  ...
  scale_y_continuous(limits = c(0, 17200), breaks = seq(0,17200,1500))+
...
#Territory plot;
testing_cumt<-subset(testing_cum, Province %in% c("NU","YT","NT") & dateformat > as.Date("2020-03-21") & dateformat < Sys.Date())
...
- **Highlights**
  ...
  - **Quebec has the highest proportion of positive tests (13.6%).**
