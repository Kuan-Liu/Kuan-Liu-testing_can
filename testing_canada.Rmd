...
- **Highlights**
  ...
  - **Quebec has rapidly increased its testing capacity. The total number of tests per 100,000 population has increased to 862 (top 2) as of Mar 31, 2020 comparing to 365 on Mar 25, 2020.**
...
map_test = tm_shape(testing_data)+tm_polygons(col="test",breaks=c(0,1000,5000,10000,50000, Inf),labels = c("<1000", "1000-5000", "5001-10000","10001-50000",">50000"), title=" ",palette ="YlOrRd")+tm_layout(title="Tests", title.size = 1,title.position=c("left","bottom"),legend.position = c(0.73,0.7))

#map stat can be grouped with fixed breaks or continuous, 
map_case = tm_shape(testing_data)+tm_polygons(col="case",breaks=c(0,1,100,500,1000,2000,Inf),labels = c("0", "1-100","101-500","501-1000","1001-2000",">2000"),title=" ",palette ="YlOrRd")+tm_layout(title="Cases", title.size = 1,title.position=c("left","bottom"),legend.position =c(0.77,0.67))

map_test_p = tm_shape(testing_data)+tm_polygons(col="test_pop",breaks=c(0,500,1000,Inf),labels = c("0-500", "501-1000",">1000"), title=" ",palette ="YlOrRd")+tm_layout(title="Tests per 100,000", title.size = 1,title.position=c("left","bottom"),legend.position =c(0.78,0.76))

map_case_p = tm_shape(testing_data)+tm_polygons(col="case_pop",breaks=c(0,1,10,20,30,Inf),labels = c("0", "1-10", "11-20","21-30",">30"), title=" ",palette ="YlOrRd")+tm_layout(title="Cases per 100,000", title.size = 1,title.position=c("left","bottom"),legend.position = c(0.84,0.7))
...
ggplot(testing_todate, aes(x=total_adjp, y=`Total cases per 100,000`, colour=Province)) +
  ...
  annotate(geom="text", x=1200, y=70, label="national test-case ratio at 4%",
              color="red") +
...
- **Highlights**
  ...
  - **Given the most recent data, Quebec completes on average 7,000 tests per day. Ontario, British Columbia and Alberta complete on average 2,000 to 3,000 tests per day.**
...
#Province plot;
testing_cum2<-subset(testing_cum, Province %in% c("AB","BC","MB","NB","NL","NS","ON","PE","QC","SK")& dateformat > as.Date("2020-03-19") & dateformat < as.Date("2020-03-31"))
...
#Territory plot;
testing_cumt<-subset(testing_cum, Province %in% c("NU","YT","NT") & dateformat > as.Date("2020-03-19") & dateformat < as.Date("2020-03-31"))
...
- **Highlights**
  ...
  - **Newfoundland and Labrador has the highest proportion of positive tests (5.9%). This can be interpreted as for every 100 tests completed (with known results) in Newfoundland and Labrador, we expect around 6 of them to be postive.**
  - **There appears to be a back log of test reporting in Prince Edward Island. HThe back log in Ontario is observed to be decreasing. The proportion of pending tests in Ontario on Mar 31, 2020 is 8.3% comparing to the same proportion on Mar 25, 2020 at 29.4%.**
