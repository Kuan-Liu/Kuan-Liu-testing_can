...
- **Highlights**
  - **Nova Scotia, Alberta, and Quebec are the top three provinces ranking by the number of tests per 100,000 population.**
...
cump<-ggplot(dfp, aes(x = forcats::fct_rev(Province), y = Total, fill = Data)) +
  geom_bar_interactive(aes(tooltip = c(paste0("Province: ", Province, "\n Count: ", abs(Total)))), 
                       stat = "identity", 
...
cumt<-ggplot(dft, aes(x = forcats::fct_rev(Territory), y = Total, fill = Data)) +
  geom_bar_interactive(aes(tooltip = c(paste0("Territory: ", Territory, "\n Count: ", abs(Total)))), stat = "identity", position = position_dodge(width = -0.5), width = 0.8, colour = "black") +
  facet_share(~Data, dir = "h", scales = "free", reverse_num = TRUE) +
  coord_flip() + 
  labs(y = "", x = " ") +
  scale_fill_manual(values = c("lightblue", "orange"))+
...
cum.pt <- cowplot::plot_grid(cump, cumt, 
                             nrow = 2, 
                             rel_heights = c(0.7,0.3), 
                             align = "v") 

girafe(code = print(cum.pt))
...
map_test <- tm_shape(testing_data)+
  tm_polygons(col="test",
              ...
              labels = c("<5000", "5000-10000", "10000-50000","50000-100000",">100000"), 
...
map_case <- tm_shape(testing_data) + 
  tm_polygons(col = "case",
              breaks = c(0, 1, 100, 500, 1000, 2000, 5000, Inf),
              labels = c("0", "1-100", "100-500", "500-1000", "1000-2000", "2000-5000", ">5000"),
...
map_test_p <- tm_shape(testing_data) +
  tm_polygons(col = "test_pop",
              breaks = c(1000, 2000, 3000, Inf),
              labels = c("1000-2000", "2000-3000",">3000"), 
              title = " ",
              palette = "YlOrRd") + 
...
map_case_p <- tm_shape(testing_data) + 
  tm_polygons(col="case_pop",
              breaks=c(0,1,20,30,40,50,Inf),
              labels = c("0", "1-20", "20-30","30-40","40-50",">50"), 
              title=" ", palette ="YlOrRd")+
...
ggplot(testing_todate, aes(x=total_adjp, y=`Total cases per 100,000`, colour=Province)) +
  ...
  annotate(geom="text", x=600, y=350, label=paste("national test-case ratio at ", TCR_CAN*100 ,"%"),
              color="red") +
  scale_x_continuous(limits = c(0, 4200), breaks = seq(0,4200,500))+
  scale_y_continuous(limits = c(0, 350), breaks = seq(0,350,20))+
...
# testing2date <-ggplot(testing_todate, aes(x = total_adjp, y = `Total cases per 100,000`, colour = Province)) +
#   geom_point_interactive(aes(tooltip = c(paste0("Province/Territory: ", Province, 
#                                                 "\n Total cases per 100,000: ", abs(`Total cases per 100,000`), 
#                                                 "\n Total cases: ", abs(total_adjp)))), size = 2) +
#   geom_text(aes(fontface=2, label=Province), hjust=-0.1,vjust=-0.5,show.legend=FALSE)+
#   geom_abline(intercept = 0, slope = TCR_CAN, color="red", 
#                  linetype="dashed", size=1)+
#   annotate(geom="text", x=300, y=180, label=paste("national test-case ratio at ", TCR_CAN*100 ,"%"),
#               color="red") +
...
# girafe(code = print(testing2date)) #Kuan:forsome reason this plot is not showing when I publish but it's there when I run on R consle. Not sure what's the problem, the first girafe worked well.
...
geom_line(size = 1) + 
  geom_point() +
  scale_y_continuous(limits = c(0, 320000), breaks = seq(0, 320000, 20000)) +
...
geom_line(size = 1)+geom_point()+
  ...
  scale_y_continuous(limits = c(0, 3800), breaks = seq(0,3800,200))+
  scale_color_manual(values=teamcolour_province)+
  scale_x_date(date_breaks="1 day",date_labels="%b %d")+
...
geom_line(size = 1)+geom_point()+
  ...
  scale_y_continuous(limits = c(0, 4200), breaks = seq(0,4200,500))+
  scale_color_manual(values=teamcolour_territory)+
  scale_x_date(date_breaks="1 day",date_labels="%b %d")+
...
#Province plot;
testing_cum2<-subset(testing_cum, Province %in% c("AB","BC","MB","NB","NL","NS","ON","PE","QC","SK")& dateformat > as.Date("2020-03-19") & dateformat < Sys.Date())
...
#Territory plot;
testing_cumt<-subset(testing_cum, Province %in% c("NU","YT","NT") & dateformat > as.Date("2020-03-19") & dateformat < Sys.Date())
...
- **Highlights**
  ...
  - **Quebec has the highest proportion of positive tests (12.9%).**
  - **There appears to be a back log of test reporting in Nunvaut. The back log in Ontario is observed to be cleared. The proportion of pending tests in Ontario has reduced significantly, at less than 5% now comparing to the same proportion on Mar 25, 2020 at 29.4%.**
